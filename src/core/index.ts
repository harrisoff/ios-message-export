...
export const exportRawPersonList = (personDatabase: sqlite3.Database) => {
  return getPersonList(personDatabase);
};
...
export const exportAggregatedList = async (personDatabase: sqlite3.Database, messageDatabase: sqlite3.Database) => {
  ...
  const resultList: Result[] = messageList.map(message => {
    ...
    const handle = handleList.find(handle => handle.ROWID === handle_id);
    if (handle) {
      const person = personList.find(person => person.c16Phone.includes(handle.id));
      if (person) {
